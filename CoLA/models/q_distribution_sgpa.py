    ...
    def forward(self, x, cur_k, mask):
        ...
        if not self.flag_sgp: 
            ...
            if self.inference_mode:
                ...
                return samples, kl, mean_out, covariance_out
...
class Transformer(torch.nn.Module):
    def __init__(self, device, vocab_size, depth, max_len, num_class, embdim, hdim, num_heads, sample_size, jitter, drop_rate, keys_len, kernel_type, flag_sgp, inference_mode=False):
        ...
        self.keys = nn.ParameterList([nn.Parameter(torch.tensor(npr.randn(num_heads,1,self.keys_len,self.embdim+1024), dtype=torch.float32))]+[nn.Parameter(torch.tensor(npr.randn(num_heads, 1, self.keys_len, self.hdim), dtype=torch.float32)) for i in range(1,self.depth)])
    ...
    def forward(self, input_data,positional, mask, data):
        x_t = []
        means = []
        covariances = []
        emb_ln, emb, keys0 = self.embedding.forward(input_data, positional, self.keys[0], self.device, data)
        x_t.append(emb)         
        z, total_kl, mean, covariance = self.sgp_layer_list[0].forward(emb_ln, keys0, mask) 
        z_prime = emb + z
        mean = emb + mean
        x_t.append(z_prime)
        means.append(mean)
        ...
        for i in range(1, self.depth):
            ...
            if self.flag_sgp:
              cur_k = self.ln(cur_k) 
            z, kl, mean, covariance = self.sgp_layer_list[i].forward(z_ln, cur_k, mask) 
            if total_kl:
                total_kl += kl
            z_prime = z_prev + z
            mean = z_prev + mean
