...
import models.sgpa
import models.q_distribution_sgpa
# import models.svdkl
...
def get_model(model_name, vocab_size, logger, args):
    ...
    if model_name == "q_distribution":
        if args.attn_type == "sgpa":
            net = models.q_distribution_sgpa.Transformer(device='cuda', vocab_size=vocab_size, depth=args.depth, max_len=100, embdim=128, num_class=args.num_classes, hdim=args.hdim, num_heads=args.num_heads, sample_size=args.sample_size, jitter=args.jitter, drop_rate=0.1, keys_len=5, kernel_type='exponential', flag_sgp=True) 
        else:
    ...
    if model_name == "vit_cola" or model_name == 'temperature_scaling' or model_name == 'mc_dropout':
        if args.attn_type == "sgpa":
