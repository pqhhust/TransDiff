...
def get_model(model_name, nb_cls, logger, args):
    ...
    if model_name == "q_distribution":
        if args.attn_type == "sgpa":
            net = models.q_distribution_sgpa.ViT(device=f'cuda:{args.gpu}', depth=args.depth, patch_size=4, in_channels=3, max_len = 64, num_class=args.nb_cls, hdim=args.hdim, num_heads=args.num_heads, sample_size=1, jitter=1e-6, drop_rate=0.1, keys_len=16, kernel_type='ard', flag_sgp=True).cuda()
        else:
    ...
    if model_name == "vit_cifar" or model_name == 'temperature_scaling' or model_name == 'mc_dropout':
        if args.attn_type == "sgpa":
